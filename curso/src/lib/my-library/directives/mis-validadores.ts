...
import { Directive, ElementRef, forwardRef, inject, Input, OnChanges, SimpleChanges } from '@angular/core';
import {
  ValidatorFn,
  AbstractControl,
  NG_VALIDATORS,
...
export function notblankValidator(control: AbstractControl): Record<string, string> | null {
  return (control.value ?? '').toString().trim() === ''
    ? { notblank: 'Requerido, no debe estar en blanco' }
    : null;
}
...
export function equalsToValidator(cntrlBind?: AbstractControl): ValidatorFn {
  ...
  return (control: AbstractControl): ValidationErrors | null => {
    ...
    return !cntrlBind || control.value !== cntrlBind.value
      ? { equalsTo: `${control.value} distinto de ${cntrlBind?.value}` }
      : null;
  };
}
@Directive({
  selector: '[equalsTo]',
  providers: [
    { provide: NG_VALIDATORS, useExisting: forwardRef(() => EqualsToValidator), multi: true },
  ],
})
